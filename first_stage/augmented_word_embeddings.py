...
corpus = dem_train + rep_train
model_base = Word2Vec(sentences=corpus, min_count=1)
model = Word2Vec(sentences=corpus, min_count=1, size=101)
assert len(model_base.wv.vocab) == len(model_base.wv.vocab) == 89220  # Constant extracted on previous analysis.
