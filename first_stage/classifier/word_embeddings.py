...
if __name__ == "__main__":
    ...
    for w in model.wv.vocab:
        if w not in z_scores:
            ...
            if z_scores[w] > 0:
                # Assigning 1 to the second to last dimension of the vector if the word has democrat statistical
                # relevance.
                model.wv.syn0[model.wv.vocab[w].index] = append(model_base.wv.syn0[model_base.wv.vocab[w].index],
                                                                [1, 0])
            elif z_scores[w] < 0:
